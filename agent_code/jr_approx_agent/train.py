...
from agent_code.jr_approx_agent.callbacks import MODEL_FILE, POSSIBLE_ACTIONS, \
    MIN_EXPLORATION_RATE, FEATURE_LENGTH, reset_memory
...
GAME_REWARDS = {
    ...
    ce.POTENTIAL_TO_EXPLODE_CRATE: 0,
    ce.POTENTIAL_TO_NOT_EXPLODE_CRATE: -0.008,
    ce.DROPPED_BOMB_NEAR_CRATE: 0.08,
    ce.ITERATION_PENALTY: -0.0001
}
...
def game_events_occurred(self, old_game_state: dict, self_action: str, new_game_state: dict, events: List[str]):
    ...
    events += list(np.repeat(ce.ITERATION_PENALTY, old_game_state['step']))
...
def end_of_round(self, last_game_state: dict, last_action: str, events: List[str]):
    ...
    if self.round_count_weights == ROUNDS_TO_UPDATE_MODEL:
